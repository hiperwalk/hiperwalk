...
static void py_sparse_matrix_delete(PyObject* capsule) {
    printf("BD, em ./pyhiperblas/hiperblas_wrapper.c: static void py_sparse_matrix_delete\n");
    smatrix_t* a = (smatrix_t*) PyCapsule_GetPointer(capsule, "py_sparse_matrix_new");
    if (a) {
        printf("Liberando smatrix_t em %p\n", (void*)a);
...
static PyObject* py_smatrixConnect(PyObject* self, PyObject* args) {
    printf("BD, em ./pyhiperblas/hiperblas_wrapper.c: static PyObject* py_smatrixConnect(PyObject* self, PyObject* args)\n");

    PyObject* capsule = NULL;
    PyObject* csr_obj = NULL;
    if (!PyArg_ParseTuple(args, "OO:py_smatrixConnect", &capsule, &csr_obj))
        return NULL;

    // Recupera o ponteiro para smatrix_t a partir do PyCapsule
    smatrix_t* smat_a = (smatrix_t*) PyCapsule_GetPointer(capsule, "py_sparse_matrix_new");
    if (!smat_a) {
        PyErr_SetString(PyExc_RuntimeError, "Erro: PyCapsule_GetPointer retornou NULL em py_smatrixConnect");
        return NULL;
    }

    // Extrai atributos do objeto csr_matrix
    ...
    // Converte para PyArrayObject (não copia memória, apenas garante formato contíguo)
    PyArrayObject* dataArr    = (PyArrayObject*) PyArray_FROM_OTF(dataObj,    NPY_FLOAT64, NPY_ARRAY_IN_ARRAY);
    PyArrayObject* indicesArr = (PyArrayObject*) PyArray_FROM_OTF(indicesObj, NPY_INT64,   NPY_ARRAY_IN_ARRAY);
    PyArrayObject* indptrArr  = (PyArrayObject*) PyArray_FROM_OTF(indptrObj,  NPY_INT64,   NPY_ARRAY_IN_ARRAY);

    if (!dataArr || !indicesArr || !indptrArr) {
        PyErr_SetString(PyExc_TypeError, "Falha ao converter atributos de csr_matrix para PyArray");
        Py_XDECREF(dataArr);
    ...
    // Ponteiros diretos para dados do NumPy (compartilhados)
    npy_float64* data     = (npy_float64*) PyArray_DATA(dataArr);
    npy_int64*   indices  = (npy_int64*)   PyArray_DATA(indicesArr);
    npy_int64*   indptr   = (npy_int64*)   PyArray_DATA(indptrArr);

    // Conecta os vetores
    smat_a->values  = data;
    smat_a->col_idx = indices;
    smat_a->row_ptr = indptr;

    smat_a->nnz = (int) smat_a->row_ptr[smat_a->nrow];

    // Mantém referência ao objeto csr_matrix para evitar dangling pointers
//    Py_INCREF(csr_obj);
 //   smat_a->py_csr_ref = csr_obj;

    // Debug opcional
    printf("   smat_a->nrow = %d\n", smat_a->nrow);
    printf("   smat_a->ncol = %d\n", smat_a->ncol);
    printf("   smat_a->nnz  = %d\n", smat_a->nnz);
    printf("   row_ptr[0] = %lld, row_ptr[nrow] = %lld\n",
    ...
    //PyArray_Descr* dtype = PyArray_DESCR((PyArrayObject*) dataArr );
    //int data_type = (dtype->kind == 'f' ? T_FLOAT : T_COMPLEX);
    int data_type = (smat_a->type == 2 ? T_FLOAT : T_COMPLEX);
    smat_a->type = data_type; 
    //printf("BD, em hiperblas_wrapper.c, em static PyObject* py_smatrixConnect(PyObject, CALL print_smatrix(smat_a); \n"); print_smatrix(smat_a);
    //py_sparse_matrix_print(smat_a);

    Py_RETURN_NONE;

    PyObject* po = PyCapsule_New((void*)smat_a, "py_smatrix_new", py_smatrix_delete);
    //printf("BD, em hiperblas_wrapper.c, em FINAL de py_smatrixConnect(PyObject, exit()\n "); exit(2222);
    return po; 
}
*/
...
/*
static PyObject* py_smatrixConnect(PyObject* self, PyObject* args){
    printf("BD1, em ./pyhiperblas/hiperblas_wrapper.c: static PyObject* py_smatrixConnect(PyObject\n");
    PyObject* csr_obj = NULL; PyObject* aSmatObj = NULL;
    if (!PyArg_ParseTuple(args, "OO:py_smatrixConnect", &aSmatObj, &csr_obj)) return NULL;
    printf("BD2, em ./pyhiperblas/hiperblas_wrapper.c: static PyObject* py_smatrixConnect(PyObject; \n");
    smatrix_t* smat_a = (smatrix_t*) PyCapsule_GetPointer(aSmatObj, "py_sparse_matrix_new");
    if (!smat_a){ printf("BD, em static PyObject* py_smatrixConnect, ERRO, "); return NULL; } 

    PyObject* dataObj    = PyObject_GetAttrString(csr_obj, "data");
    PyObject* indicesObj = PyObject_GetAttrString(csr_obj, "indices");
    PyObject* indptrObj  = PyObject_GetAttrString(csr_obj, "indptr");
    PyObject* shapeObj   = PyObject_GetAttrString(csr_obj, "shape");
    if (!dataObj || !indicesObj || !indptrObj) { PyErr_SetString(PyExc_TypeError, "Objeto não é csr_matrix válido"); return NULL; }

    PyArrayObject* indptrArr32  = (PyArrayObject*) PyArray_FROM_OTF(indptrObj,  NPY_INT32,   NPY_ARRAY_INOUT_ARRAY);
    smat_a->row_ptr = (npy_int32 *) PyArray_DATA(indptrArr32);   
    printf("BD3, em ./pyhiperblas/hiperblas_wrapper.c: static PyObject* py_smatrixConnect(PyObject; exit()\n");exit(128+13);
    PyArrayObject* indicesArr32 = (PyArrayObject*) PyArray_FROM_OTF(indicesObj, NPY_INT32,   NPY_ARRAY_INOUT_ARRAY);
    PyArrayObject* dataArr64    = (PyArrayObject*) PyArray_FROM_OTF(dataObj,    NPY_FLOAT64, NPY_ARRAY_INOUT_ARRAY);
    PyArrayObject* shapeArr     = (PyArrayObject*) PyArray_FROM_OTF(shapeObj,   NPY_INT32,   NPY_ARRAY_INOUT_ARRAY);

    npy_int64 *shape = (npy_int32*) PyArray_DATA(shapeArr);

    smat_a->values  = (npy_float64 *) PyArray_DATA(dataArr64) ; 
    smat_a->col_idx = (npy_int32 *) PyArray_DATA(indicesArr32); 

    smat_a->nrow = (npy_int64) shape[0];
    smat_a->ncol = (npy_int64) shape[1];
    smat_a->nnz = smat_a->row_ptr[smat_a->nrow];//-1;
...
static PyObject* py_smatrixConnect(PyObject* self, PyObject* args)
{
    ...
    if (!dataObj || !indicesObj || !indptrObj || !shapeObj) {
        PyErr_SetString(PyExc_TypeError, "Objeto não é csr_matrix válido");
        return NULL;
    }

    // --- Verifica tipos e contiguidade (sem cópias) ---
    PyArrayObject *indptrArr  = (PyArrayObject*) indptrObj;
    PyArrayObject *indicesArr = (PyArrayObject*) indicesObj;
    PyArrayObject *dataArr    = (PyArrayObject*) dataObj;

    if (!PyArray_Check(indptrArr) || PyArray_TYPE(indptrArr) != NPY_INT32 || !PyArray_ISCARRAY(indptrArr)) {
        PyErr_SetString(PyExc_TypeError, "indptr deve ser np.int32 contíguo");
        return NULL;
    }
    if (!PyArray_Check(indicesArr) || PyArray_TYPE(indicesArr) != NPY_INT32 || !PyArray_ISCARRAY(indicesArr)) {
        PyErr_SetString(PyExc_TypeError, "indices deve ser np.int32 contíguo");
        return NULL;
    }
    if (!PyArray_Check(dataArr) || PyArray_TYPE(dataArr) != NPY_FLOAT64 || !PyArray_ISCARRAY(dataArr)) {
        PyErr_SetString(PyExc_TypeError, "data deve ser np.float64 contíguo");
        return NULL;
    }

    // Shape: tupla (nrow, ncol)
    npy_intp *shape = PyArray_SHAPE((PyArrayObject*) PyArray_FROM_OTF(shapeObj, NPY_INT64, NPY_ARRAY_IN_ARRAY));

    // --- Compartilhamento real de memória ---
    smat_a->row_ptr = (npy_int32*) PyArray_DATA(indptrArr);
    smat_a->col_idx = (npy_int32*) PyArray_DATA(indicesArr);
    smat_a->values  = (npy_float64*) PyArray_DATA(dataArr);

    smat_a->nnz  = smat_a->row_ptr[smat_a->nrow];
    smat_a->isPacked = 1;

    printf("CSR conectado: nrow=%lld, ncol=%lld, nnz=%lld\n",
           (long long) smat_a->nrow, (long long) smat_a->ncol, (long long) smat_a->nnz);
